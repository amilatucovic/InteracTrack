...
class FeedbackDB(Base):
    ...
    # JSON za dodatne podatke - PROMIJENJENO IME!
    feedback_metadata = Column(JSON, nullable=False, default=lambda: {})  # <-- OVO JE ISPRAVNO
    ...
    def to_dict(self):
        ...
        return {
            ...
            'feedback_metadata': self.feedback_metadata,  # <-- ISPRAVLJENO
            'created_at': self.created_at.isoformat() if self.created_at else None
        }
...
class AgentLearningDB(Base):
    ...
    # JSON za dodatne metrike - PROMIJENJENO IME!
    learning_metrics_data = Column(JSON, nullable=False, default=lambda: {})  # <-- OVO JE ISPRAVNO
    ...
    def to_dict(self):
        ...
        return {
            ...
            'learning_metrics_data': self.learning_metrics_data,  # <-- ISPRAVLJENO
    ...
    def test_connection(self):
        ...
        try:
            with self.get_session() as session:
                ...
                # Proveri tabele
                table_names = [tbl.name for tbl in self.engine.table_names()] if hasattr(self.engine, 'table_names') else []
